...
def show_model(
    ...
    def animate_2d(i: int) -> Any:
        ...
        """
        current_time = dataset["t"].values[i]
        ...
        tx.set_text(f"t = {current_time:.2f}")
    ...
    if dataset.y.size == 1:
        ...
        im, tx, line = _setup_2d_plot(fig=fig, cv0=frames[0], dataset=dataset)
        ani = animation.FuncAnimation(
...
def _setup_2d_plot(fig, cv0, dataset):
    """
    ...
    im = ax.imshow(cv0, origin="lower", interpolation="gaussian")
    ax.scatter(1, 5, s=3, color="black")
    ax.scatter(1, 6, s=3, color="black")
    ax.scatter(2, 5, s=3, color="black")
    fig.colorbar(im, cax=cax)

    ax2 = fig.add_subplot(122)
    ax2.plot(dataset["t"].values, dataset.sel(x=1, y=5)["n"].values)
    line = ax2.axvline(dataset["t"].values[0], ymin=-10, ymax=10)
